...
def inference(loader, model, device):
    ...
    feature_vector = []
    labels_vector = []
    for step, (x, (input_ids, token_type_ids, attention_mask), y) in enumerate(loader):
        x = x.to(device)
        input_ids = input_ids.to("cuda")
        token_type_ids = token_type_ids.to("cuda")
        attention_mask = attention_mask.to("cuda")
        with torch.no_grad():
            c = model.forward_cluster(x, input_ids, token_type_ids, attention_mask)
        c = c.detach()
...
if __name__ == "__main__":
    ...
    if args.dataset == "CIFAR-10":
        ...
        train_dataset = CUB200(
            root_path=f"{args.dataset_dir}/CUB_200_2011",
            split="train",
            transform=transform.Transforms(size=args.image_size).test_transform,
        )
        test_dataset = CUB200(
